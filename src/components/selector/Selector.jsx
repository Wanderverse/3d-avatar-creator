...
export default function Selector({
  ...
  const getRestrictions = () => {
    ...
    for (const prop in traitRestrictions) {
      ...
      getAsArray(traitRestrictions[prop].restrictedTraits).map((traitName) => {
        ...
        traitRestrictions[traitName].restrictedTraits = [
          ...new Set([...traitRestrictions[traitName].restrictedTraits, ...[prop]]),
      ...
      getAsArray(traitRestrictions[prop].restrictedTypes).map((typeName) => {
        ...
        typeRestrictions[typeName].restrictedTraits = [
          ...new Set([...typeRestrictions[typeName].restrictedTraits, ...[prop]]),
        ] // make sure to add prop as restriction
      })
    }
    ...
    for (const prop in templateInfo.typeRestrictions) {
      ...
      typeRestrictions[prop].restrictedTypes = [
        ...new Set([...typeRestrictions[prop].restrictedTypes, ...getAsArray(templateInfo.typeRestrictions[prop])]),
      ]
      ...
      getAsArray(templateInfo.typeRestrictions[prop]).map((typeName) => {
        ...
        typeRestrictions[typeName].restrictedTypes = [
          ...new Set([...typeRestrictions[typeName].restrictedTypes, ...[prop]]),
        ] // make sure to add prop as restriction
      })
    }
  ...
  const loadOptions = (options, filterRestrictions = true) => {
    ...
    if (filterRestrictions) options = filterRestrictedOptions(options)
  ...
  const itemAssign = (itemData) => {
    ...
    models.map((m) => {
      ...
      if (getAsArray(templateInfo.blinkerTraits).indexOf(traitData.trait) !== -1) blinkManager.addBlinker(vrm)
      ...
      vrm.scene.traverse((child) => {
        ...
        if (child.isMesh) {
          ...
          if (cullingIgnore.indexOf(child.name) === -1) cullingMeshes.push(child)

          if (child.geometry.boundsTree == null) child.geometry.computeBoundsTree({ strategy: SAH })
